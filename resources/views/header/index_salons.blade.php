                    ...
                    <?php $delay=$delay+0.3;?>
                        ...
                        <div class="features-item wow fadeInUp" data-wow-delay="{{$delay}}s" >
                           <img src="{{ Voyager::image( $exhibition->image ) }}" width="100%" alt=""/> 
                            ...
                            </br>/</br>{{ \Carbon\Carbon::parse($exhibition->from)->format('m')}}</div><div style="padding-left: 10px; font-weight: 400; padding-top: 10px" class="uppercase">{{ $exhibition->name }}</div></div>
                        </div>
                        <!-- End Features Item -->
                        @endforeach
